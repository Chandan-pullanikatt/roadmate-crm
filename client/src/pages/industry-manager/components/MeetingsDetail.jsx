...
const meetingType = (action) => {
  if (!action) return 'Meeting';
  if (action.includes('virtual')) return 'Virtual';
  if (action.includes('direct')) return 'Direct';
  if (action.includes('done')) return 'Done';
  if (action.includes('scheduled')) return 'Scheduled';
  return 'Meeting';
};

const meetingVariant = (action) => {
  if (!action) return 'purple';
  if (action.includes('virtual')) return 'blue';
  if (action.includes('direct')) return 'teal';
  if (action.includes('done')) return 'green';
  return 'purple';
};

const feedbackLabel = (note, action) => {
  if (!note) return null;
...
const MeetingsDetail = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['activities', 'meetings', page],
    queryFn: () => dashboardApi.getActivities('meetings', { page, limit: 30 }).then(r => r.data),
    staleTime: 2 * 60 * 1000,
    placeholderData: prev => prev
  });
    ...
    <div className="space-y-6 animate-in fade-in duration-700 pb-12">
      ...
      <div className="card shadow-lg shadow-purple/5 border-border/40 overflow-hidden">
        <div className="px-8 pt-6 pb-3">
          <h3 className="text-sm font-black text-text-primary uppercase tracking-tight">All Meeting Records</h3>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse min-w-[1000px]">
            ...
            <tbody className="divide-y divide-border/40">
              {filtered.map((a, idx) => {
                const type = meetingType(a.action);
                const variant = meetingVariant(a.action);
                const feedback = feedbackLabel(a.note, a.action);
                const isDone = a.action?.includes('done');
                const isScheduled = a.action?.includes('scheduled') || a.action?.includes('virtual') || a.action?.includes('direct');
                ...
                return (
                  <tr key={a._id || idx} className="hover:bg-purple-light/5 transition-colors group">
                    ...
                    <td className="px-6 py-4 text-center">
                      <Tag
                        variant={isDone ? 'green' : isScheduled ? 'blue' : 'amber'}
                        label={isDone ? 'Completed' : isScheduled ? 'Scheduled' : 'Pending'}
                        className="text-[9px] font-black px-3 py-1 rounded-lg uppercase tracking-tighter"
                      />
                    </td>
                    <td className="px-6 py-4">
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={7} className="px-8 py-16 text-center text-text-muted italic">
                    {search ? `No meetings matching "${search}"` : 'No meeting records found'}
                  </td>
                </tr>
              )}
            </tbody>
