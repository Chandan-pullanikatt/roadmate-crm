    ...
    >
      <div className="space-y-8 py-2">
        ...
        <div className="flex items-center gap-4 p-4 bg-surface2/30 rounded-2xl border border-border">
          ...
          <div className="flex gap-6 pr-4 border-l border-border pl-8">
            <div className="text-center">
              ...
              <div className="text-[12px] text-text-muted uppercase font-bold">Paid Balance</div>
            </div>
            <div className="text-center">
              <div className="text-[18px] font-black text-teal">{balance?.approvedThisMonth || 0}</div>
              <div className="text-[12px] text-text-muted uppercase font-bold">Used This Month</div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="text-[13px] font-black text-text-muted uppercase tracking-[0.2em]">Recent Applications</div>
          
          <div className="border border-border rounded-2xl overflow-hidden bg-white">
            <table className="w-full text-left">
              <thead>
                <tr className="bg-surface2/50 border-b border-border">
                  <th className="p-4 pl-6 text-[13px] font-black uppercase text-text-muted tracking-widest">Type / Dates</th>
                ...
                {isLoading ? (
                  ...
                  history.map(leave => (
                    <tr key={leave._id} className="hover:bg-surface2/30 transition-all">
                      ...
                      <td className="p-4 text-[14px] font-medium text-text-secondary max-w-xs truncate" title={leave.reason}>
                        {leave.reason}
                      </td>
                    </tr>
                  ))
